...
APVER='channelmonitor3.py 26.06.2013'  # give parameters if not running on android. development stopped.
...
# 18.04.2013 counter restore in case of PIC reset. do not use dochannels table for that! also some transactions aligned.
# 24..28.04.2013 prooviks marko soovitusel ip aadressiks 0.0.0.0 panna udp socketi tegemisel- sobib, ei soltu enam 3g/wlan/hotspot modest!
# 28.04.2013 MBsta[] introduced, to monitor modbus devices based on modbus_comm() returncode. datagram number also to be monitored, up to hourly?
# 01.05.2013 several exiting cases via stop=1 added. logging to file added. no rotation or file size check yet!
# 06.05.2013 sql table replacement 
# 08.05.2013 ioplaat vajab ai7,8 toolehakkamiseks 271 korduvat kirjutamist! enne 0000 ja siis 0030 voi 003 vms
# 12.05.2013 chk setup kirjutamine r275! I1W ja ECW saatmisega jama
# 13.05.2013 added cmd:CONFIG to write configuration into modbus slaves according to asetup/setup. reports also actual setup as R<mba>.<regadd>
# 14.05.2013 fixed looping config from yesterday
# 17.05.2013 cfgnum, avoid endless proxy connection retry on reboot if no tcpconn possible
# 20.05.2013 added cmd:CONFIG, run. found F1S, R1S wrong (2) before first F1W or R1W change. must be sumstatus handling. 
# 22.05.2013 sumstatus=0 in the beginning of each svc. does not report F1W, R1W at all! local web recognizes di changes. 
# 17.06.2013 stop olgu globaalne, et udp send probleemi korral restart toimuks!
# 19.06.2013 added report_channelconfig(), executed with cmd:VARLIST. so far counters not included!
...
# 26.06.2013 tgz support in cmd:pull, directories can be created now in d4c
# STOP # The development continues in channelmonitor_pm.py, using pymodbus 
